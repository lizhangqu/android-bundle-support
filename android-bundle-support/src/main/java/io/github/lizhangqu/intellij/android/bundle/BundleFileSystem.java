...
public class BundleFileSystem extends ArchiveFileSystem {
    ...
    private static final Key<Boolean> APKZIP_KEY = Key.create("android.zip.within.bundle");

    public static BundleFileSystem getInstance() {
        return (BundleFileSystem) VirtualFileManager.getInstance().getFileSystem(PROTOCOL);
    }
    ...
    public VirtualFile extractAndGetContentRoot(VirtualFile file) {
        ...
        try {
            ...
            Logger.getInstance(BundleFileSystem.class).warn("IOException while extracting zip file from APK", e);
            return null;
        }
        try (InputStream is = new ByteArrayInputStream(file.contentsToByteArray())) {
            Files.copy(is, tempFile.toPath(), StandardCopyOption.REPLACE_EXISTING);
        } catch (IOException e) {
            Logger.getInstance(BundleFileSystem.class).warn("IOException while copying contents of zip file to temp file", e);
            return null;
        }
